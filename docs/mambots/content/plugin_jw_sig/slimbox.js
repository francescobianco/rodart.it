...
var Lightbox = {
	init: function(options) {
		...
		this.header = new Element('div').setProperties({id:'lbHeader'}).injectInside(this.center);
		
		this.image = new Element('div').setProperty('id', 'lbImage').injectInside(this.center);
		this.prevLink = new Element('a').setProperties({id: 'lbPrevLink', href: '#'}).setStyle('display', 'none').injectInside(this.image);
		this.nextLink = this.prevLink.clone().setProperty('id', 'lbNextLink').injectInside(this.image);
		
		this.realimage = new Element('div').setProperty('id', 'lbRealImage').injectInside(this.image);
	...
	click: function(link) {
		...
		this.anchors.each(function(el){
			if(el.rel == link.rel) {
				...
				if(j == images.length) {
					images.push([el.href, el.title, $(el).getProperty('header')]);
	...
	nextEffect: function() {
		...
		case 1:
			...
			this.image.setStyles({background: 'none', width: (clientWidth()-50)+'px'});
			
			this.image.style.height = this.prevLink.style.height = this.nextLink.style.height = this.preload.height+'px';
			this.image.style.height = (clientHeight()-100)+"px";
			
			this.realimage.setHTML('<center><img src="'+this.images[this.activeImage][0]+'" height="'+(clientHeight()-170)+'"></center>');
			
			this.caption.setHTML(this.images[this.activeImage][1] || '');
			this.number.setHTML((this.images.length == 1) ? '' : 'Image '+(this.activeImage+1)+' of '+this.images.length);
			this.header.setHTML(this.images[this.activeImage][2] || '');
